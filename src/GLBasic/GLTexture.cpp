...
void GLTexture::loadTexture(char* fileName)
{
    ...
    image= SOIL_load_image(fileName,&width,&height,0,SOIL_LOAD_RGBA); // THIS IS CAUSING ERROR


    if(!image)std::cout<<"ERROR: image not found"<<std::endl; // error check
    ...
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP);  // how to wrap on object in x direction
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);  // how to wrap on object in y direction
